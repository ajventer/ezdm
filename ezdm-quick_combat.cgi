#!/usr/bin/env python
from ezdm_libs.util import smart_input,load_json,highlight,rolldice,dice_list,say,title,heal_dice
from ezdm_libs.character import Character,list_chars
import sys
from ezdm_libs import gui
if gui():
...
def load_chars():
    ...
    while more=='y':
        ...
        characters.append(Character(load_json('characters',clist[selected]),auto,QuietDice=False))
...
def select_target(player_index,no_exclude=False):
    ...
    selected=smart_input("%s is attacking: please select target" %characters[player_index].displayname(),validentries=selectlist(exclude))
...
def attack_mods(player_index):
    ...
    if gui():
       #TODO - custom modifiers in GUI mode
        selected=eg.multchoicebox("Select combat modifiers",title=title(),choices=custom_mods.keys())
        if selected:
            for selection in selected:
                selected_mods[selection]=custom_mods[selection]
...
def do_attack(player_index):
    ...
    while attack_number <= num_attacks and target_alive:
        ...
        who=highlight("%s is attacking %s with %s!" %(characters[player_index].displayname(),characters[target_index].displayname(),weapon))
        who="%s\n%s" %(who,highlight("Attack %s of %s" %(attack_number,num_attacks),clear=False))
        attack_number += 1        
...
def do_spell_complete(player_index):
    ...
    if action == 'heal':
        characters[player_index].spell_target.heal(heal_dice(characters[player_index].autoroll()))
...
def do_round():
    ...
    for upchar in initiative:
        ...
        if not characters[player_index] in in_combat():
            continue #This happens if a character dies during a round - he still has initiative but isn't in the fight anymore         
        ...
        action=smart_input("%s:What happens" %who,validentries=actionlist)        
        if action == "Attack":
            if characters[player_index].is_casting():
               characters[player_index].interrupt_cast()     
            do_attack(player_index)
        elif action == "Other ability":
            if len(characters[player_index].conditionals()) == 0:
                print "%s has no special abilities" %characters[player_index].displayname()
                continue
            ability=smart_input("Ability being tried",validentries=characters[player_index].conditionals().keys())
            if characters[player_index].tryability(ability):
                say("%s succeeded with %s" %(characters[player_index].displayname(),ability))
            else:
                say("%s failed at %s" %(characters[player_index].displayname(),ability))
        elif action == "Flee":
            if characters[player_index].is_casting():
               characters[player_index].interrupt_cast()     
